...
export default function AttendanceModel() {
  ...
  const overtimeConfirmationModal = useRef(null);
  const reasonModal = useRef(null);
  ...
  const handleClockOutSubmit = () => {
    clockOut(clockOutPayload)
      .then(() => {
        ...
        fetchTodaysAttendance();
        reasonModal.current.querySelector("form").submit();
      })
      .catch((err) => {
        if (err.message.includes("distance")) {
          window.overtimeSubmissionCheckoutModal.querySelector("form").submit();
          window.overtimeSubmissionConfirmationModal
            .querySelector("form")
            .submit();
          window.oopsOutOfRangeModal.showModal();
          // Show out of range modal
        } else {
          showAlert("ERROR", err);
        }
      })
  ...
  /**
   * Handles when the overtime confirmation buttons are clicked
   */
  const handleConfirmOvertimeSubmission = (event) => {
    if (event.target.value === "Yes") {
      setClockOutPayload({ ...clockOutPayload, confirmation: true });
      setClockOutState({ ...clockOutState, loading: true });
      overtimeConfirmationModal.current.querySelector("form").submit();
      window.overtimeSubmissionCheckoutModal.showModal();
    } else {
      setClockOutPayload({ ...clockOutPayload, confirmation: false });
      overtimeConfirmationModal.current.querySelector("form").submit();
      handleClockOutSubmit();
    }
  };
  ...
  return {
    ...
    handlers: {
      ...
      clockOut: {
        ...
        confirmation: handleConfirmOvertimeSubmission,
    ...
    refs: {
      ...
      clockOut: {
        confirmation: overtimeConfirmationModal,
        reason: reasonModal,
      },
      logs: {
        early: earlyRef,
